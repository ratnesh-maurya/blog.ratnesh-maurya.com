-- get_utm_analytics returned each byRef row as { ref: <value>, count: <n> }.
-- "ref" collides with React's reserved `ref` prop: recharts spreads each raw
-- data entry as element props when rendering bars (Bar.js), so a data key
-- literally named `ref` gets interpreted as a React ref instead of a plain
-- prop, crashing with "Expected ref to be a function, an object returned by
-- React.createRef(), or undefined/null." Renaming the JSON field to
-- "refValue" avoids the collision; no other byX breakdown uses a reserved
-- prop name.
...
begin
  ...
  select jsonb_build_object(
    ...
    'byRef', (
      ...
      from (
        select ref as "refValue", count(*)::int as count
        from public.utm_hits
        where (created_at at time zone 'utc')::date between p_from and p_to
...
comment on function public.get_utm_analytics(date, date) is
  'Aggregated UTM counts by source, medium, campaign, content, term, ref (as refValue), path, and day. total/daily count only hits with a real utm_source (excludes direct/organic traffic).';

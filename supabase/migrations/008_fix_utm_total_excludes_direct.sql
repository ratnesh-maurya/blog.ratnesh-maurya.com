-- Fix get_utm_analytics: 'total' counted every utm_hits row, including hits
-- with no utm_source (direct/organic traffic), unlike the bySource/byMedium/
-- byCampaign breakdowns which already filter those out. This made "Today's
-- UTM Visits" on the dashboard include direct traffic and exceed real page
-- views. 'total' now matches the same "has a real UTM source" definition as
-- the other aggregates in this function.
...
comment on function public.get_utm_analytics(date, date) is
  'Aggregated UTM counts by source, medium, campaign, content, term, ref, path, and day. total/daily count only hits with a real utm_source (excludes direct/organic traffic).';
